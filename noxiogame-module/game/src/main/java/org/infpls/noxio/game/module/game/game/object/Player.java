...
public abstract class Player extends Mobile {
  ...
  */
  public void generateUpdateData(final StringBuilder sb) {
    ...
    final String name = c!=null?c.user.display:""; /* @TODO: fucking oof, change this. massively inefficent and wasteful of bandwidth */
  ...
  public List<Mobile> hitTest(final Vec2 p, final float r) {
    ...
    for(int i=0;i<game.objects.size();i++) {
      final GameObject obj = game.objects.get(i);
      if(obj != this && obj.is(Types.MOBILE) && !(obj.is(Types.PICKUP) && !((Pickup)obj).canAttack(team))) {
